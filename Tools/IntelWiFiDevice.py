...
class Intel_IWL5300_API():
    ...
    def drawPhase(self, aFilePath):
        app = QtWidgets.QApplication([])
        
        mainWindow = QtWidgets.QMainWindow()
        mainWindow.resize(800, 600)
        mainWindow.setWindowTitle('CSI phase flow')
        ...
        specWid = pg.PlotWidget(title='phase')
        specItem = specWid.getPlotItem()

        lay.addWidget(specWid)
        specWid.setYRange(0, 50)
        mainWindow.show()
                
        #read csi data from local file
        ...
        while cur < (fileLength - 1):
            ...
            if code == 187:
                ...
                if sum(perm) != triangle[nrx - 1]:
                    ...
                    csi = _deviceSymbolData[count - 1]['csi']
                    
                   # print "---------print csi data-----------"
                   # print csi
                    ...
                    if flag:
                       flag = 0 
                       smoothCSImtrx = self.getSmoothCSI(csiMatrix)    
                       #print smoothCSImtrx
                    '''
                    #print csiMatrix
                    #removedCSI = self.getSmoothCSI(csiMatrix)
                    #csiMatrixList.append(removedCSI)
                    csiMatrixList.append(csiMatrix)
       
            #print csiMatrixList
            #print "-----------------------"            
            _csiMatrix = csiMatrixList[0]
            ...
            specItem.clearPlots()
            specItem.plot(np.unwrap(np.angle(_csiMatrix[:, 0])), pen=(255, 0, 0))
            specItem.plot(np.unwrap(np.angle(_csiMatrix[:, 1])), pen=(0, 255, 0))
            specItem.plot(np.unwrap(np.angle(_csiMatrix[:, 2])), pen=(0, 0, 255))
            app.processEvents()
            
            #wrapped phase
            #specItem.clearPlots()
            #specItem.plot(np.angle(_csiMatrix[:, 0]), pen=(255, 0, 0))
            #specItem.plot(np.angle(_csiMatrix[:, 1]), pen=(0, 255, 0))
            #specItem.plot(np.angle(_csiMatrix[:, 2]), pen=(0, 0, 255))
            #app.processEvents()
                        
            
            """
            for index in range(len(csiMatrixList)):
                _csiMatrix = csiMatrixList[index]
                specItem.clearPlots()
                specItem.plot(np.unwrap(np.angle(_csiMatrix[:, 0])), pen=(255, 0, 0))
                specItem.plot(np.unwrap(np.angle(_csiMatrix[:, 1])), pen=(0, 255, 0))
                specItem.plot(np.unwrap(np.angle(_csiMatrix[:, 2])), pen=(0, 0, 255))
    ...
    def getSmoothCSI(self, csiMatrix):
...
class IntelDeviceService(ExtractorService):
    ...
    def draw_phase(self, filePath):
        self._mDriver.drawPhase(filePath)
